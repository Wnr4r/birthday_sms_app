...
$check = mysql_num_rows($result);
	if ($check >= 1)
	{
		//initialize an array to store sent messages details
		$sent = array();
		$failedMsg = array();
		...
		$mailMsg = "Details of msg are: " . print_r($sent, true);
		$mailMsg = wordwrap($mailMsg,70);
		if (mail ('syntocode@syntocode.com', 'Sent Msgs Details', $mailMsg)){
			echo "<br/> A MAIL WAS SENT "; }
			else { echo "<br/> MAIL SENDING FAILED";}
	}
		else
			{
